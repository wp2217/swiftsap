    ...
    ZOARequestAction: {
      ZOARequestAction_HttpPort: {
        createRequest: function (args) {
          ...
          var requestJSON = args.requestJSON
          var reqJSON = {
          ...
          console.log('requestJSON: start----->\r\n')
          console.log(requestJSON)
          console.log('\r\nrequestJSON: <-----end\r\n')

          console.log('reqJSON: start----->\r\n')
          console.log(reqJSON)
          console.log('\r\nreqJSON: <-----end\r\n')

          if (requestJSON == undefined) {
            console.log('传入参数(requestJSON)不能为空\r\n')
            return { out: 'E|' + "传入参数(requestJSON)不能为空" }
          }

          if (reqJSON.headJSON == undefined) {
            console.log('传入参数(headJSON)不能为空\r\n')
            return { out: 'E|' + "传入参数(headJSON)不能为空" }
          }

          var allUser = requestJSON.approvers
          ...
          function zcreateRequest() {
            ...
            mysql.transaction(function (trx) {
              ...
              //单据处理
              if (request.iduser != '') {
                mysql.insert(reqJSON, 'idreqjson')
                  .into('REQJSON')
                  .transacting(trx)
                  .then(function (ids) {
                    if (ids[0] > 0) {
